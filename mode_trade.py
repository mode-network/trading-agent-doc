...
# Beta CCXT Exchange for Mode Trade
...
class modetrade(Exchange, ImplicitAPI):
        ...
        return self.deep_extend(
            super(modetrade, self).describe(),
            {
                ...
                "urls": {
                    ...
                    # "fees": [
                    #     "https://trade.mode.network/en/orderly",
                    # ],
                    # "referral": {
                    #     "url": "https://trade.mode.network/en/trade?ref=CCXT",
                    #     "discount": 0.05,
                    # },
                },
                "api": {
        ...
        for i in range(0, len(tokenRows)):
            ...
            for j in range(0, len(networks)):
                ...
                precision = self.parse_precision(
                    self.safe_string(network, "decimals")
                )
    ...
    def fetch_trades(
        ...
        if limit is not None:
            request["limit"] = limit
        response = self.v1PublicGetPublicMarketTrades(
    ...
    def parse_funding_rate(
        self, fundingRate, market: Market = None
    ) -> FundingRate:
        #
        #         {
        ...
        market = self.market(symbol)
        nextFundingTimestamp = self.safe_integer(
            fundingRate, "next_funding_time"
        )
        estFundingRateTimestamp = self.safe_integer(
            fundingRate, "est_funding_rate_timestamp"
        )
        lastFundingRateTimestamp = self.safe_integer(
            fundingRate, "last_funding_rate_timestamp"
        )
        fundingTimeString = self.safe_string(
            fundingRate, "last_funding_rate_timestamp"
        )
        nextFundingTimeString = self.safe_string(
            fundingRate, "next_funding_time"
        )
        millisecondsInterval = Precise.string_sub(
            nextFundingTimeString, fundingTimeString
        )
        return {
            "info": fundingRate,
            "symbol": market["symbol"],
            ...
            "previousFundingRate": self.safe_number(
    ...
    def fetch_funding_rate(self, symbol: str, params={}) -> FundingRate:
        ...
        response = self.v1PublicGetPublicFundingRateSymbol(
            self.extend(request, params)
        )
        #
        # {
    ...
    def fetch_funding_rate_history(
        ...
        paginate, params = self.handle_option_and_params(
            params, "fetchFundingRateHistory", "paginate"
        )
        if paginate:
        ...
        request, params = self.handle_until_option(
            "end_t", request, params, 0.001
        )
        response = self.v1PublicGetPublicFundingRateHistory(
            self.extend(request, params)
        )
        #
        # {
    ...
    def fetch_order_book(
        ...
        response = self.v1PrivateGetOrderbookSymbol(
            self.extend(request, params)
        )
        #
        # {
    ...
    def parse_order(self, order: dict, market: Market = None) -> Order:
        ...
        timestamp = self.safe_integer_n(
            order, ["timestamp", "created_time", "createdTime"]
        )
        orderId = self.safe_string_n(
            order, ["order_id", "orderId", "algoOrderId"]
        )
        ...
        amount = self.safe_string_2(
            order, "order_quantity", "quantity"
        )  # This is base amount
        cost = self.safe_string_2(
        ...
        filled = self.omit_zero(
            self.safe_value_2(order, "executed", "totalExecutedQuantity")
        )
        average = self.omit_zero(
            self.safe_string_2(
                order, "average_executed_price", "averageExecutedPrice"
            )
        )
        ...
        if childOrders is not None:
            ...
            if innerChildOrdersLength > 0:
                ...
                takeProfitPrice = self.safe_number(
                    takeProfitOrder, "triggerPrice"
                )
                stopLossPrice = self.safe_number(stopLossOrder, "triggerPrice")
        lastUpdateTimestamp = self.safe_integer_2(
            order, "updatedTime", "updated_time"
        )
    ...
    def create_order_request(
        ...
        clientOrderId = self.safe_string_n(
            params, ["clOrdID", "clientOrderId", "client_order_id"]
        )
        if clientOrderId is not None:
            request["client_order_id"] = clientOrderId
        if triggerPrice is not None:
            request["trigger_price"] = self.price_to_precision(
                symbol, triggerPrice
            )
            ...
            if stopLoss is not None:
                stopLossPrice = self.safe_number_2(
                    stopLoss, "triggerPrice", "price", stopLoss
                )
                stopLossOrder: dict = {
                    "side": closeSide,
                    "algo_type": "TP_SL",
                    "trigger_price": self.price_to_precision(
    ...
    def create_orders(self, orders: List[OrderRequest], params={}):
        ...
        *contract only* create a list of trade orders
        ...
        for i in range(0, len(orders)):
            ...
            triggerPrice = self.safe_string_2(
                orderParams, "triggerPrice", "stopPrice"
            )
            ...
            if isConditional:
                raise NotSupported(
                    self.id + " createOrders() only support non-stop order"
                )
    ...
    def edit_order(
        ...
        triggerPrice = self.safe_string_n(
            params,
            ["triggerPrice", "stopPrice", "takeProfitPrice", "stopLossPrice"],
        )
        if triggerPrice is not None:
    ...
    def cancel_order(self, id: str, symbol: Str = None, params={}):
        ...
        clientOrderIdUnified = self.safe_string_2(
        ...
        if trigger:
            if isByClientOrder:
                ...
                request["order_id"] = id
                response = self.v1PrivateDeleteAlgoOrder(
                    self.extend(request, params)
                )
        else:
            if isByClientOrder:
                ...
                response = self.v1PrivateDeleteClientOrder(
                    self.extend(request, params)
                )
            else:
                request["order_id"] = id
                response = self.v1PrivateDeleteOrder(
                    self.extend(request, params)
                )
        #
        # {
    ...
    def cancel_orders(self, ids: List[str], symbol: Str = None, params={}):
        ...
        if clientOrderIds:
            ...
            response = self.v1PrivateDeleteClientBatchOrder(
                self.extend(request, params)
            )
        else:
            request["order_ids"] = ",".join(ids)
            response = self.v1PrivateDeleteBatchOrder(
                self.extend(request, params)
            )
        #
        # {
    ...
    def cancel_all_orders(self, symbol: Str = None, params={}):
        ...
        response = None
        if trigger:
            response = self.v1PrivateDeleteAlgoOrders(
                self.extend(request, params)
            )
        else:
    ...
    def fetch_order(self, id: str, symbol: Str = None, params={}):
        ...
        params = self.omit(
            params,
            ["stop", "trigger", "clOrdID", "clientOrderId", "client_order_id"],
        )
        response = None
        if trigger:
            if clientOrderId:
                ...
                request["oid"] = id
                response = self.v1PrivateGetAlgoOrderOid(
                    self.extend(request, params)
                )
        else:
            if clientOrderId:
                ...
                request["oid"] = id
                response = self.v1PrivateGetOrderOid(
                    self.extend(request, params)
                )
        #
        # {
    ...
    def fetch_orders(
        ...
        paginate, params = self.handle_option_and_params(
            params, "fetchOrders", "paginate"
        )
        if paginate:
            return self.fetch_paginated_call_incremental(
                "fetchOrders", symbol, since, limit, params, "page", maxLimit
            )
        request: dict = {}
        market: Market = None
        params = self.omit(params, ["stop", "trigger"])
        ...
        response = None
        if isTrigger:
            response = self.v1PrivateGetAlgoOrders(
                self.extend(request, params)
            )
        else:
    ...
    def fetch_order_trades(
        ...
        response = self.v1PrivateGetOrderOidTrades(
            self.extend(request, params)
        )
        #
        # {
    ...
    def fetch_my_trades(
        ...
        paginate = False
        paginate, params = self.handle_option_and_params(
            params, "fetchMyTrades", "paginate"
        )
        if paginate:
            return self.fetch_paginated_call_incremental(
                "fetchMyTrades", symbol, since, limit, params, "page", 500
            )
        request: dict = {}
        market: Market = None
        if symbol is not None:
        ...
        return self.safe_ledger_entry(
            {
                ...
                "timestamp": timestamp,
                "datetime": self.iso8601(timestamp),
                "type": self.parse_ledger_entry_type(
                    self.safe_string(item, "type")
                ),
                "info": item,
            },
            currency,
    ...
    def parse_transaction(
        ...
        fee = self.parse_token_and_fee_temp(
            transaction, "fee_token", "fee_amount"
        )
        ...
        return {
            ...
            "currency": code,
            "status": self.parse_transaction_status(
                self.safe_string(transaction, "status")
            ),
    ...
    def fetch_deposits(
        ...
        return self.fetch_deposits_withdrawals(
            code, since, limit, self.extend(request, params)
        )
    ...
    def fetch_deposits_withdrawals(
        ...
        """
        request: dict = {}
        currencyRows = self.get_asset_history_rows(
            code, since, limit, self.extend(request, params)
        )
        currency = self.safe_value(currencyRows, 0)
        rows = self.safe_list(currencyRows, 1)
    ...
    def withdraw(
        self, code: str, amount: float, address: str, tag=None, params={}
    ) -> Transaction:
        """
        make a withdrawal
        ...
        currency = self.currency(code)
        verifyingContractAddress = self.safe_string(
            self.options, "verifyingContractAddress"
        )
        ...
        msg = self.eth_encode_structured_data(
        ...
        response = self.v1PrivatePostWithdrawRequest(
            self.extend(request, params)
        )
        #
        #     {
    ...
    def fetch_position(self, symbol: Str, params={}):
        ...
        response = self.v1PrivateGetPositionSymbol(
            self.extend(request, params)
        )
        #
        # {
    ...
    def sign(
        ...
        if access == "public":
            ...
            if (method == "POST" or method == "PUT") and (
                ...
                isSandboxMode = self.safe_bool(
                    self.options, "sandboxMode", False
                )
                if not isSandboxMode:
                    brokerId = self.safe_string(
                        self.options, "brokerId", "CCXT"
                    )
                    if path == "batch-order":
            ...
            if secret.find("ed25519:") >= 0:
                ...
                secret = parts[1]
            signature = self.eddsa(
                self.encode(auth), self.base58_to_binary(secret), "ed25519"
            )
            headers["orderly-signature"] = self.urlencode_base64(
    ...
    def handle_errors(
        ...
        if not success:
            ...
            self.throw_broadly_matched_exception(
                self.exceptions["broad"], body, feedback
            )
